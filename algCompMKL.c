...
void addmcompact(double *Bc, double *A, int n, int nm1, double *R){
int i,j,k;
//memcpy(R,A,n*n*sizeof(double));
for(i=1;i<nm1-1;i++){
	for(k=max(0,2-i);k<=min(4,nm1+2-i-1);k++){
		R[(i-2+k)*n+i] = A[(i-2+k)*n+i]+Bc[5*1+k];
		//printf("i = %d \t (i-2+k)*n+i = %d \t 5*1+k = %d \n",i,(i-2+k)*n+i,5*1+k);
	}
}
for(j=0;j<3;j++){
	R[j*n+0] = A[j*n+0]+Bc[0*5+2-j];
	R[(n-3+j)*n+nm1-1]=A[(n-3+j)*n+nm1-1]+Bc[0*5+j];
...
void computeQcompact(double *LRAc, int n, int nm1, double *Q){
int i,j,k,count = 4;
//Upper triangular
for(i=0;i<n;i++){
	for(j=i;j<=min(nm1,i+3);j++){
		for(k=1;k<=count;k++){
			//printf("i=%d    j=%d    k=%d \n",i,j,k);
			Q[j*n+i] += LRAc[i*4+(3-count+k)]*LRAc[j*4+k-1];
		}
	count--;
	}
count = 4;
...
for(i=nm1;i>=0;i--){
	for(j=i-1;j>=max(0,i-3);j--){
		for(k=1;k<=count;k++){
			...
			Q[j*n+i] += LRAc[j*4+(3-count+k)]*LRAc[i*4+k-1];
		}
	count--;
	}
count = 3;
